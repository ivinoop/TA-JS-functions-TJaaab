...
function calculateMoviesToWatch(age, moviesPerWeek) {
  const maxAge = 90;
  let moviesPerMonth = moviesPerWeek * 4;
  let moviesPerYear = moviesPerMonth * 12;
  while ((age > 18) && (age <= maxAge)) {
    return ((maxAge - age) * moviesPerYear);
  }
}
...
function celsiusToFahrenheit(tempCelsius) {
  let tempFahr = (tempCelsius * 1.8) + 32;
  return(`Temperature in Fahrenheit is ${tempFahr}`);  
}
...
function fahrenheitToCelsius(tempFahr) {
  let tempCelsius = (tempFahr - 32) * 0.55;
  return(`Temperature in Celsius is ${tempCelsius}`);
}
